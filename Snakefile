...
rule all:
	input:
		...
		expand("bam_files/{sample}.bam.bai", sample=paired_samples + single_samples),
...
rule fastqc:
        ...
        shell:
                "mkdir -p fastqc_report && fastqc {input.fastq} -o {params.fastqc_dir}"

rule multiqc:
	input:
		fastqc_reports = expand("fastqc_report/{sample}_fastqc.html", sample=SAMPLES),
		fastqc_zips = expand("fastqc_report/{sample}_fastqc.zip", sample=SAMPLES)
	output:
		"multiqc_report/multiqc_report.html"  # Output path for the multiqc report
	shell:
		"multiqc -f fastqc_report -o multiqc_report"
...
rule sort_sam: # works for both bowtie2 and hisat2
	...
	shell:
		"mkdir -p bam_files && samtools sort -o {output} {input}"
...
rule index_bam: # works for both bowtie2 and hisat2
	...
	shell:
		"mkdir -p bam_files && samtools index {input}"
...
rule stringtie:
	...
	params:
		label="{sample}"                                                                                                                
	shell:
		"mkdir -p gtf_files && stringtie -G {input.annotation} -o {output} -l {params.label} {input.bamfile} -e"
